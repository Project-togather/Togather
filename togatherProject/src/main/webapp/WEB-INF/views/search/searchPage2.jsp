...
<body>
							...
							<div class="modal-body">
                                ...
                                <div class="map_wrap">
                                    ...
                                    <div id="menu_wrap" class="bg_white">
                                        <div class="option">
                                            <div>
                                                <!-- Modify the input field to trigger search automatically -->
                                                검색 : <input type="text" id="keyword" value="${ hiddenLocation }" size="15" oninput="searchPlaces()">
                                            </div>
                                        </div>
                                        <hr>
                                        <ul id="placesList"></ul>
                                    ...
                                    <br>
                                    <div>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp 검색 결과 주소 : <input id="selectedAddress" name="selectedAddress" type="text"></div>
                                    <input type="hidden" id="hiddenLocation" name="hiddenLocation" value="${ hiddenLocation }" <c:if test="${hiddenLocation != null}">checked</c:if>>
                                    <br>
                                </div>
						</div>
                ...
                for ( var i=0; i<places.length; i++ ) {
                    ...
                    (function(marker, title, placeId) {
                        ...
                        if($(this).hasClass("addorange")){
                            ...
                            $(this).addClass("addorange");
                            $("#classLocation").val(title.address_name);
                            // 여기서 "selectedAddress" 입력 필드를 업데이트합니다
                            document.getElementById("selectedAddress").value = title.address_name;
                        }
    ...
    // Add a click event listener to change marker color when clicked
    kakao.maps.event.addListener(marker, 'click', function () {
        map.setCenter(marker.getPosition());

        // Remove the 'addorange' class from other markers
        for (var i = 0; i < markers.length; i++) {
            markers[i].setZIndex(i + 1); // Adjust zIndex to maintain proper layering
            markers[i].removeClass('addorange');
        }

        // Add the 'addorange' class to the clicked marker
        marker.addClass('addorange');
    });
    ...
    return marker;
            ...
            // 선택된 주소 값을 "selectedAddress" 입력 필드의 value로 설정
            ...
            function copyAddressToHiddenInput() {
            // 검색 결과 주소 input 요소와 hidden input 요소를 가져옵니다.
            var addressInput = document.getElementById("selectedAddress");
            var hiddenLocationInput = document.getElementById("hiddenLocation");

            // 검색 결과 주소 input 요소의 값을 가져와 hidden input 요소에 설정합니다.
            hiddenLocationInput.value = addressInput.value;
        }
